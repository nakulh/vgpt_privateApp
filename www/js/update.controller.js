...
var resUrl = "http://192.168.1.105:8080" + "/Laravel/VGPT/resources/";
app.controller('UpdateCtrl', function($scope, UpdateService, DbServiceSettings, DbItemAdd, $cordovaDevice, $timeout, $cordovaSQLite, $http, $cordovaFileTransfer, DbTest){
  ...
  $scope.checkQuestionUpdates = function(){
    ...
    $cordovaSQLite.execute(db, query, [uuid]).then(function(result){
      ...
      if(result.rows.item(0).accessMethod == "intranet"){
        url = "http://192.168.1.105:8080" + "/Laravel/VGPT/public/api/v1/question/" + count; /// add count
      }
      else{
  ...
  $scope.checkVideoUpdates = function(){
    ...
    $cordovaSQLite.execute(db, query).then(function(result){
      ...
      if(result.rows.item(0).accessMethod == "intranet"){
        url = "http://192.168.1.105:8080" + "/Laravel/VGPT/public/api/v1/video/" + count;
      }
      else{
  ...
  $scope.checkDppUpdates = function(){
    ...
    $cordovaSQLite.execute(db, query).then(function(result){
      ...
      var urld = "";
      if(result.rows.item(0).accessMethod == "intranet"){
        urld = "http://192.168.1.105:8080" + "/Laravel/VGPT/public/api/v1/dpp/batch";
      }
      else{
      ...
      $http({
        ...
        for(var x = 0; x < res.length; x++){
          ...
          var url = "http://192.168.1.105:8080/" + "/Laravel/VGPT/resources/" + res[x].URL;
